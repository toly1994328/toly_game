...
class World3D extends CustomPainter {
  ...
  void drawLinePoint(Canvas canvas, double d) {
    ...
    Path pathY = Path()
      ..moveTo(0, y)
      ..relativeLineTo(x, 0);
    DashPainter(span: 6, step: 6)
        .paint(canvas, pathY, paint..color = Colors.cyanAccent);
  }
  ...
  void drawStep3(Canvas canvas) {
    ...
    TextStyle style = TextStyle(color: Colors.white, fontSize: 16);
    TextPainter(
        text: TextSpan(text: "p:(160,40,80)", style: style),
        textDirection: TextDirection.ltr)
      ..layout()
      ..paint(canvas, p1.translate(-20, 4));
    canvas.drawCircle(p1, 4, Paint()..color = Colors.cyanAccent);

    Path path2D = Path();
    path2D.moveTo(p1.dx, 0);
    path2D.lineTo(p1.dx, p1.dy);

    path2D.moveTo(0, p1.dy);
    path2D.lineTo(p1.dx, p1.dy);
    const DashPainter(span: 6, step: 6)
        .paint(canvas, path2D, paint..color = Colors.cyanAccent);
  }
